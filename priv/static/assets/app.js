  ...
  var require_topbar = __commonJS({
    "vendor/topbar.js"(exports, module) {
      (function(window2, document2) {
        ...
        var canvas2, progressTimerId, fadeTimerId, currentProgress, showing, addEvent = function(elem, type, handler) {
          ...
          var lineGradient = ctx.createLinearGradient(0, 0, canvas2.width, 0);
          for (var stop in options.barColors)
            lineGradient.addColorStop(stop, options.barColors[stop]);
          ctx.lineWidth = options.barThickness;
          ctx.beginPath();
          ctx.moveTo(0, options.barThickness / 2);
          ctx.lineTo(Math.ceil(currentProgress * canvas2.width), options.barThickness / 2);
...
`, template.innerHTML.trim());
  ...
  Hooks2.ChatScrollManager = {
    mounted() {
      ...
      this._trackReadPosts = () => {
        if (this.el.dataset.smartUnread !== "true")
          return;
      ...
      this._observer = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && this.el.dataset.hasMore === "true" && !this._loadingMore) {
  ...
  Hooks2.Grimoire = {
    mounted() {
      ...
      this.popover.addEventListener("keydown", (e) => {
        var _a;
        if (this.addInput.style.display !== "none")
          return;
        ...
        if (e.key === "ArrowRight" || e.key === "ArrowDown") {
          ...
          e.preventDefault();
          (_a = items[this.selectedIndex]) == null ? void 0 : _a.click();
        } else if (e.key === "Escape") {
          e.preventDefault();
          this.closePopover();
        }
  ...
  Hooks2.LibraryProlegomenonTOC = {
    ...
    buildTOC() {
      ...
      const slugCounts = {};
      headings.forEach((h) => {
        const base = h.textContent.trim().toLowerCase().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-");
      ...
      const annotationsLink = annotationsEl ? `<div class="library-prolog-toc-footer"><a class="library-toc-link" href="#folio-toc">annotations \u2193</a></div>` : "";
  ...
  Hooks2.LibraryComposer = {
    ...
    initSortable() {
      ...
      this.sortable = new Sortable(list, {
        ...
        onStart: (event) => {
          ...
          this.groupSnapshot = groupId ? Array.from(list.querySelectorAll(`[data-group-id="${groupId}"]`)).map((el) => el.dataset.entryId) : null;
        },
        onEnd: (event) => {
          ...
          if (draggedGroupId && this.groupSnapshot) {
            const groupEls = this.groupSnapshot.map((id) => list.querySelector(`[data-entry-id="${id}"]`)).filter(Boolean);
          ...
          const orderedIds = Array.from(items).map((el) => el.dataset.entryId).filter(Boolean).join(",");
    ...
    initRangeHover() {
      this._clearRangePreview = () => {
        document.querySelectorAll(".browser-range-preview").forEach((el) => el.classList.remove("browser-range-preview"));
      };
      this._onMouseover = (e) => {
        const panel = document.getElementById("scene-browser-panel");
        const anchorId = panel == null ? void 0 : panel.dataset.rangeAnchorId;
        if (!anchorId)
          return;
        const postEl = e.target.closest("[id^='browser-post-']");
        if (!postEl)
      ...
      this._onMouseout = (e) => {
        var _a;
        if (!((_a = e.relatedTarget) == null ? void 0 : _a.closest("[id^='browser-post-']"))) {
          this._clearRangePreview();
        }
      };
    ...
    initRangeClick() {
      this._onRangeClick = (e) => {
        ...
        if (e.shiftKey)
          return;
        if (e.target.closest("[data-from-btn]"))
          return;
        const postEl = e.target.closest("[id^='browser-post-']");
        if (!postEl)
          return;
        ...
        const postId = postEl.id.replace("browser-post-", "");
        const sceneEl = postEl.closest("[data-scene-id]");
        const sceneId = sceneEl == null ? void 0 : sceneEl.dataset.sceneId;
        if (postId && sceneId) {
    ...
    mounted() {
      ...
      this.channel.onmessage = (event) => {
        if (event.data.type === "pong") {
          ...
          audio.volume = 0;
          manualPlayBtn == null ? void 0 : manualPlayBtn.classList.remove("hidden");
      ...
      this.handleEvent("queue_update", ({ now_playing, queue, queue_length }) => {
        ...
        if (queueList) {
          queueList.innerHTML = queue.map((item, i) => `<li>${i + 1}. ${item.title} (by ${item.queued_by})</li>`).join("");
        }
      });
  ...
  Hooks2.PinnedScenes = {
    mounted() {
      ...
      this.pushEvent("restore_pinned_scenes", { ids });
      this.handleEvent("update_pinned_scenes", ({ ids: ids2 }) => {
        localStorage.setItem("pinnedSceneIds", JSON.stringify(ids2));
      });
    }
  };
  Hooks2.TemenosMoveReporter = {
    mounted() {
  ...
  Hooks2.Temenos = {
    mounted() {
      this.handleEvent("loadAvatarMenu", (e) => {
        ...
        avatarmenu.createWheel(["\u{1F4D6}", "\u27A1\uFE0F", "\u2694\uFE0F", "\u{1F3F9}", "\u2795", "\u{1F6E1}\uFE0F", "\u2716\uFE0F"]);
      ...
      this.handleEvent("loadTemenosMenu", (e) => {
        ...
        if (e.gm) {
          if (e.current_showhide) {
            temenosmenu.createWheel(["\u2795", "\u{1F4CD}", "\u{1F648}", "\u{1F3AF}"]);
            ...
            temenosmenu.createWheel(["\u2795", "\u{1F4CD}", "\u{1F441}\uFE0F", "\u{1F3AF}"]);
            temenosmenu.navItems[0].navigateFunction = () => menuClick("temenosPlace");
            temenosmenu.navItems[1].navigateFunction = () => menuClick("temenosCounter");
            temenosmenu.navItems[2].navigateFunction = () => menuClick("temenosToggleHide");
            temenosmenu.navItems[3].navigateFunction = () => menuClick("temenosTarget");
          ...
          temenosmenu.createWheel(["\u2795", "\u{1F4CD}", "\u{1F3AF}"]);
          temenosmenu.navItems[0].navigateFunction = () => menuClick("temenosPlace");
          temenosmenu.navItems[1].navigateFunction = () => menuClick("temenosCounter");
      ...
      this.handleEvent("loadCardMenu", (e) => {
        ...
        cardmenu.createWheel(["\u{1F5D1}\uFE0F", "\u27A1\uFE0F", "\u2194\uFE0F", "\u{1F4D8}", "\u{1F451}\u{1F4D8}", "\u{1F4D6}", "\u2716\uFE0F"]);
    ...
    mounted() {
      ...
      this.el.addEventListener("touchmove", (e) => {
        if (touchState.isPanning && e.touches.length === 1) {
          ...
          const rect2 = this.el.getBoundingClientRect();
          const centerX = (e.touches[0].clientX + e.touches[1].clientX) / 2 - rect2.left;
          const centerY = (e.touches[0].clientY + e.touches[1].clientY) / 2 - rect2.top;
  ...
  Hooks2.RumorMap = {
    mounted() {
      ...
      this.applySearch = (rawQuery) => {
        ...
        const matchedPrimaryConnIds = /* @__PURE__ */ new Set();
        document.querySelectorAll("#nodes-container [data-node-id]").forEach((el) => {
      ...
      this.getWorldTransform = () => {
        const match = this.world.style.transform.match(/translate3d\(([^,]+)px,\s*([^,]+)px,\s*[^)]+\)\s*scale\(([\d.]+)\)/);
        if (!match)
      ...
      this.drawConnection = (c, zoom) => {
        ...
        const isDimmed = this.matchedPrimaryConnIds !== null && !this.matchedPrimaryConnIds.has(c.conn.id.toString());
        const ctx = this.ctx;
        ctx.save();
      ...
      this.lineTippy = window.tippy ? window.tippy(document.createElement("div"), { trigger: "manual", placement: "top", offset: [0, 12], allowHTML: false }) : null;
      ...
      this.handleEvent("remove_deleted_node_connections", ({ node_id }) => {
        ...
        toRemove.forEach((connId) => {
          if (this.connections[connId])
            this.removeConnection(connId, true);
        });
      });
    ...
    drawConnectionFromData(conn) {
      ...
      if (existing && !existing.secondaryId) {
        ...
        if (primaryEntry && !primaryEntry.isMerged && primaryEntry.color === this.getConnColor(conn)) {
    ...
    removeConnection(connId, skipPromotion = false) {
      ...
      if (entry.isMerged) {
        ...
        if (this.pairIndex[pairKey])
          this.pairIndex[pairKey].secondaryId = null;
    ...
    destroyed() {
      if (this._resizeObserver)
        this._resizeObserver.disconnect();
      if (this.lineTippy)
        this.lineTippy.destroy();
      this.connections = {};
      this.pairIndex = {};
    }
  };
  function getMousePosition(canvas2, e) {
...
`;
  ...
  Hooks2.BBSCopyLink = {
    mounted() {
      this.handleClick = async (e) => {
        var _a;
        e.preventDefault();
        const postId = (_a = this.el.closest("[id^=post-]")) == null ? void 0 : _a.id;
        if (postId) {
  ...
  Hooks2.BBSCopyQuote = {
    mounted() {
      this.handleClick = (e) => {
        e.preventDefault();
        this.pushEvent("copy_quote", { post_id: this.el.dataset.postId });
      };
      this.el.addEventListener("click", this.handleClick);
      this.handleEvent("bbs-copy-quote", async ({ text }) => {
        try {
  ...
  Hooks2.NPCQuickSwitch = {
    ...
    _loadPins() {
      try {
        return JSON.parse(localStorage.getItem("npc-pins") || "[]");
      } catch (e) {
        return [];
      }
    },
    ...
    _getPresets() {
      try {
        return JSON.parse(this.el.dataset.presets || "[]");
      } catch (e) {
        return [];
      }
    },
    ...
    _render() {
      ...
      const filtered = presets.filter((p) => {
        if (!query)
          return true;
        return (p.name || "").toLowerCase().includes(query) || (p.narrative_author_name || "").toLowerCase().includes(query);
      });
      ...
      this._filteredItems.forEach((preset, idx) => {
        ...
        item.type = "button";
        item.className = "npc-qs-item" + (isPinned ? " npc-qs-item--pinned" : "") + (idx === this._focusedIdx ? " npc-qs-item--focused" : "");
        ...
        const avatarHtml = preset.avatar_path ? `<img src="${this._esc(preset.avatar_path)}" class="npc-qs-avatar" alt="" />` : `<div class="npc-qs-avatar npc-qs-avatar--placeholder"></div>`;
        item.innerHTML = `
        ${avatarHtml}
      ...
      `;
        item.addEventListener("click", (e) => {
          if (e.target.closest(".npc-qs-pin-btn"))
            return;
