defmodule Strangepaths.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      # Start the Ecto repository
      Strangepaths.Repo,
      # Start the Telemetry supervisor
      StrangepathsWeb.Telemetry,
      # Start the PubSub system
      {Phoenix.PubSub, name: Strangepaths.PubSub},
      # Start the Endpoint (http/https)
      StrangepathsWeb.Endpoint,
      # Start a worker by calling: Strangepaths.Worker.start_link(arg)
      # {Strangepaths.Worker, arg}
      Strangepaths.Cards.Ceremony,
      Strangepaths.Presence
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: Strangepaths.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    StrangepathsWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
